...
for round in {1..3}; do
    ...
    # Para cada um dos 3 experimentos - inicializa o cache para todo o experimento
    for exp_index in "${!DESTINATIONS[@]}"; do
        DEST="${DESTINATIONS[exp_index]}"
        DEST_DIR=$(dirname "$DEST")
        PYTHON_FILE="$DEST"
        ...
        cd "$DEST_DIR"
        echo "Inicializando cache para $PYTHON_FILE - Modo intra-args"
        
        # Apaga qualquer cache anterior
        rm -rf "$DEST_DIR/.speedupy/"
        
        # Setup inicial para o modo intra-args (cache é mantido entre argumentos)
    ...
    # Para cada um dos 5 argumentos
    for arg_index in {0..4}; do
        echo "Argumento índice $arg_index - Modo intra-args"
        # Para cada um dos 3 experimentos
        for exp_index in "${!DESTINATIONS[@]}"; do
            DEST="${DESTINATIONS[exp_index]}"
            DEST_DIR=$(dirname "$DEST")
            ...
            # Executa o script Python com o argumento no modo 'manual', mantendo o cache entre argumentos
            python3.12 $PYTHON_FILE $(echo "$ARG") --exec-mode manual | tail -n 1 | cut -d':' -f2 >> ${OUTPUT_FILES["${exp_index}_intra_args"]}
    ...
    for exp_index in "${!DESTINATIONS[@]}"; do
        DEST="${DESTINATIONS[exp_index]}"
        DEST_DIR=$(dirname "$DEST")
        echo "Limpando cache para $DEST após todos os argumentos - Modo intra-args"
        rm -rf "$DEST_DIR/.speedupy/"
    done
done
