...
ARGUMENTS_0=("50" "100" "150" "200" "250") # look_and_say
ARGUMENTS_1=("5000" "10000" "15000" "20000" "25000") # gauss_legendre_quadrature
...
for arg_index in {0..4}; do
    ...
    # Para cada um dos 3 experimentos - inicializa o cache para este argumento
    for exp_index in "${!DESTINATIONS[@]}"; do
        DEST="${DESTINATIONS[exp_index]}"
        DEST_DIR=$(dirname "$DEST")
        PYTHON_FILE="$DEST"
        ...
        cd "$DEST_DIR"
        echo "Inicializando cache para $PYTHON_FILE argumento $arg_index - Modo intra-args"
        
        # Cria um diretório temporário para armazenar o cache entre rodadas
        CACHE_STORAGE_DIR="$DEST_DIR/.speedupy_cache_${exp_index}_${arg_index}"
        mkdir -p "$CACHE_STORAGE_DIR"
        
        # Se já existir um cache armazenado, restaura para .speedupy
    ...
    # Para cada uma das 3 rodadas
    for round in {1..3}; do
        echo "Rodada $round para argumento $arg_index - Modo intra-args"
        ...
        for exp_index in "${!DESTINATIONS[@]}"; do
            ...
            cd "$DEST_DIR"
            echo "Executando $PYTHON_FILE com argumento $ARG - Rodada $round - Modo intra-args"
    ...
    # Após todas as rodadas para este argumento, armazena os caches
    for exp_index in "${!DESTINATIONS[@]}"; do
        DEST="${DESTINATIONS[exp_index]}"
        DEST_DIR=$(dirname "$DEST")
        CACHE_STORAGE_DIR="$DEST_DIR/.speedupy_cache_${exp_index}_${arg_index}"
        ...
        cd "$DEST_DIR"
        echo "Armazenando cache para experimento $exp_index, argumento $arg_index"
        
        # Armazena o cache para uso nas próximas rodadas
        rm -rf "$CACHE_STORAGE_DIR/.speedupy" 2>/dev/null
        if [ -d ".speedupy" ]; then
            cp -r ".speedupy" "$CACHE_STORAGE_DIR/"
        fi
        
        # Limpa o cache atual (o próximo argumento começará fresh)
...
# Limpeza final - remove todos os diretórios de cache temporários
for exp_index in "${!DESTINATIONS[@]}"; do
    DEST="${DESTINATIONS[exp_index]}"
    DEST_DIR=$(dirname "$DEST")
